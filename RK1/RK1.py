...
class TeacherCourse:
    """Преподаватели курсов (для связи многие-ко-многим)"""
...
def main():
    ...
    one_to_many = [(t.surname, t.salary, c.name)
                   for c in courses
                   for t in teachers
                   if t.course_id == c.id]
    ...
    many_to_many = [(t.surname, t.salary, course_name)
    ...
    for i in res_1:
        print(f'Курс: {i[2]}, Преподаватель: {i[0]}, Зарплата: {i[1]}')

    print('\nЗадание А2')
    res_2_unsorted = []
    for c in courses:
        course_teachers = list(filter(lambda i: i[2] == c.name, one_to_many))
        # Если на курсе есть преподаватели
        if len(course_teachers) > 0:
            # Суммируем зарплаты всех преподавателей курса
            total_salary = [salary for _, salary, _ in course_teachers]
            total_salary_sum = sum(total_salary)
            res_2_unsorted.append((c.name, total_salary_sum))

    # Сортируем по суммарной зарплате (по убыванию)
